...
def draw_boxes_and_label_on_image_cv2(img, class_label_map, class_boxes_map):
    ...
    for c, boxes in class_boxes_map.items():
        for box in boxes:
            ...
            cv2.rectangle(img, (bb_left, bb_top), (bb_width, bb_height), unique_color, 2) #検出領域
            ...
            cv2.rectangle(img, (text_org[0] - 5, text_org[1] + base_line - 5),
                          (text_org[0] + ret_val[0] + 5, text_org[1] - ret_val[1] + 5), unique_color, 2) #テキスト挿入部分
            # this rectangle for fill text rect
            cv2.rectangle(img, (text_org[0] - 5, text_org[1] + base_line - 5),
                          (text_org[0] + ret_val[0] + 5, text_org[1] - ret_val[1] + 5),
                          unique_color, -1)
            cv2.putText(img, text_label, text_org, cv2.FONT_HERSHEY_DUPLEX, 0.6, (255,255,255), 1) #テキスト
    return img
